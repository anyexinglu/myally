...
> 状态：持续更新。最近整理：2026-07-17。主计划已升级为v3.1。
...
本决策没有改变POC-1A的真实完成边界：当前代码只有完整回复和消息记录，Hermes-lite、Memory Observer、记忆检索、Skill路由和工具调用均尚未实现。
...
主计划与交接文档发生冲突时，先标明冲突，不静默覆盖。v3.1已成为当前执行基线；OpenSpec初始化后，结构性变更先形成change并确认，再实施、验证、同步生效规格和归档。
