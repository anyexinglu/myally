...
> 状态：持续更新。最近整理：2026-07-21。主计划已升级为v3.2。
> 用途：保存主计划之外仍会影响后续决策的讨论结论、依据、取舍与待验证项，保证任何 Agent 不依赖聊天记录即可接手。

## 0. 2026-07-21 精选页技能板块改造为内置技能专区

**结论：**精选页「技能」板块从外部 AI 工具卡片（复制官网链接）改造为**内置技能专区**：12 个预设角色的 AI 能力包（周报助手、写作润色、翻译官、家常菜厨子、育儿顾问、健康问答、合同法务、Excel公式、旅行规划师、起名先生、情绪树洞、健身教练），2 列宫格展示（参考腾讯元宝专家列表），点击技能直接 `wx.switchTab` 进聊天，分身以该角色服务。OpenSpec change 为 `openspec/changes/featured-skill-zone/`，已通过严格校验。

**依据：**用户在精选页上线后明确确认新方向——技能板块应为本产品可直接使用的能力入口，给分身加能力包，而不是导流外部工具。`wx.switchTab` 不能带参，跨 tab 传值只能走 `getApp().globalData.pendingSkill`。角色预设属于模型上下文而非用户消息：若前缀拼进用户文本会被 `memoryEligible=true` 污染 Memory Observer 输入并落库，因此注入点选为服务端 Agent 系统提示词追加块（主 `SYSTEM_PROMPT` 之后、明确标注优先级低于系统安全规则）。

**影响：**

- 新增 `miniprogram/data/skills.json`（12 技能，字段 `id/name/emoji/oneLiner/tags/welcomeMessage/systemPrompt`）；`tools.json` 文件保留在 `data/` 但不再被精选页引用；信息板块（场景方案）完全未动。
- 精选页 `useSkill` 暂存 pendingSkill 并 switchTab；聊天页 `onShow` 消费：插入 `welcomeMessage` 为本地 assistant 消息（不落库、不进历史），技能存入 `data.activeSkill`，注入后立即清空 pendingSkill 防重复触发；`onLoad` 缓存 `_ready` Promise 解决会话加载与开场白插入的时序竞争；进入临时对话模式清空 activeSkill。无 pendingSkill 时聊天页行为与之前完全一致。
- 发送链路：payload 携带 `skillPrompt`（重试沿用 draft 原值）；`normalizeInput` 校验 ≤1000 字；`ConversationService → AgentOrchestrator → CloudBaseModelAdapter.next` 注入系统提示词追加块；skillPrompt 不落库、不进记忆观察器、不改变工具白名单与 Policy Engine。`packages/conversation`、`packages/agent` 的云端部署副本（`domain.js`、`agent.js`）已同步。
- `scripts/validate-project.js` 增加 skills.json 12 技能字段断言与 pendingSkill/skillPrompt 链路断言（必需文件 33 个）；`tests/conversation.test.js` 新增 2 个用例（超长拒绝＋注入块仅在场时出现；透传 agent 且不落库）。
- **云函数 `conversations` 已改动，真实环境必须重新部署后技能角色才生效**；未部署时旧函数静默忽略 skillPrompt 字段，行为退化为普通聊天，不报错。

**实测：**`npm run check` 通过；`npm test` 53/55（2 个失败仍为先前存在的 `release-hardening` 先行红灯，与本次无关）；`npm run demo`、`npm run demo:agent` 通过；`npx openspec validate featured-skill-zone --strict` 通过。

**待验证 / 已知边界：**

1. 宫格视觉、点击进聊天、开场白展示、角色化回答需在微信开发者工具/真机复查（先重新部署 `conversations`），未计为视觉或端到端验收通过。
2. 激活技能仅随当前页面会话存续，小程序重进后角色消失；如需会话级持久角色再单独立 change。
3. 技能为客户端传入提示词，已做长度硬校验且注入位置在主安全规则之后；技能内容本身无敏感信息。
4. 与活跃 change `release-review-hardening` 的正式包页面清单规划仍需归档时对齐（既有冲突，未变化）。
5. AI 超市 `tools.json` 已无页面引用，后续如确认不再需要可单独立 change 删除，本次保留。

## 0. 2026-07-21 「精选」tab 与原生 tabBar 上线

**结论：**小程序从自绘胶囊底部导航切换为原生三 tab：聊天（home）/ 精选（featured）/ 我的（mine）。新增 `pages/featured` 原生 TypeScript 页面，顶部 segment 切换「技能」（AI 工具卡片，含名称、一句话介绍、上手难度、免费标签、「复制官网」按钮调 `wx.setClipboardData`）与「信息」（场景方案卡片，含标题、适用人群、点击展开/收起分步步骤，工具名纯文本）。OpenSpec change 为 `openspec/changes/featured-tab-and-native-tabbar/`，已通过严格校验。

**依据：**用户在微信对话中直接明确要求新增第三个 tab「精选」并复用 AI 超市数据，视为已确认。数据源是 AI 超市项目（`ai-supermarket-miniapp/src/data/`）手工编写的真实数据 `tools.json`（14 条）与 `solutions.json`（7 条），字节级原样复制到 `miniprogram/data/`，未改写内容。tabBar 不引图片资源（纯文字），符合用户指令；`watch`、`devtest` 保留在 pages 但不进 tabBar。

**影响：**

- `app.json` 新增 `tabBar`（选中色 `#2aaa80`，白底）；featured 注册在 home 之后。
- home/mine 页移除自绘 `.bottom-nav`；首页输入框 `composer-zone` 从 `bottom: calc(144rpx + env(safe-area-inset-bottom))` 改为 `bottom: 0`（原生 tabBar 页视口自动排除 tabBar 高度），消息列表 `calc(100vh - 212rpx)` 逻辑不变，聊天页布局理论上不变形。
- 跨 tab 跳转从 `wx.redirectTo` 改为 `wx.switchTab`（home→mine、mine→home、watch→home/mine）；watch 为非 tab 页，保留自绘导航作返回入口；devtest 仍由 mine 页 `wx.navigateTo` 进入。
- `miniprogram/tsconfig.json` 增加 `resolveJsonModule` 与 `esModuleInterop` 以支持 JSON import（小程序运行时 CommonJS require 原生支持 JSON）。
- `scripts/validate-project.js` 增加 featured 四件套与 data 两个 JSON 为必需文件，并断言 tabBar 恰好为 home/featured/mine 三项。
- 未动云函数、home 聊天逻辑、任何 API 或外部跳转；不涉及真实 AppID/OpenID/密钥。

**实测：**`npm run check` 通过（32 必需文件、5 页面、tabBar 断言、TypeScript 含 JSON import 无错误）；`npm run demo` 与 `npm run demo:agent` 通过；OpenSpec 严格校验通过。`npm test` 为 51/53：2 个失败用例在 `tests/release-hardening.test.js`，是尚未实施的活跃 change `release-review-hardening` 的先行红灯断言（about 页、移除 watch/devtest、AI 生成标识），在改动前基线同样失败，与本次无关。

**待验证 / 已知冲突：**

1. 微信开发者工具与真机上的三 tab 视觉、输入框位置、复制官网 toast 尚未人工复查，不计为视觉验收通过。
2. **方向冲突需显式对齐：**活跃 change `release-review-hardening` 规划正式包移除 watch/devtest、只留 home/mine/about 三页，而本次按用户指令保留 watch/devtest 并新增 featured tab。两个 change 归档时必须先确认正式包页面清单（featured 是否进正式包、watch/devtest 是否移除），再同步规格与 `tests/release-hardening.test.js` 的页面断言，不得静默二选一。
3. AI 超市数据后续若更新，需手动重新复制两个 JSON 并 diff 确认，暂无自动同步机制。
...
### 0.4 2026-07-19 文字/语音回复链路修复

**结论：**文字链路的模型默认值、记忆模块导出和远端`conversations`部署已修复。微信同声传译插件因当前个人主体无法申请且开发者工具返回`89260`，已从代码和`app.json`移除，不再作为语音方案。

**依据：**腾讯云官方一句话识别支持60秒内、3MB内的AAC短音频；个人腾讯云账号开通服务后每月有5000次免费额度。新账号默认关闭后付费，且后付费关闭时免费额度耗尽将停服。云函数运行时会注入短期腾讯云身份，因此无需向用户索要或在仓库保存长期云密钥。参考：<https://cloud.tencent.com/document/product/1093/35646>、<https://cloud.tencent.com/document/product/1093/35686>、<https://cloud.tencent.com/document/product/583/30228>。

**影响：**语音改为“原生录音→临时云存储→`asr`云函数→一句话识别→立即删除录音→复用文字`send()`”。服务未开通、权限不足或额度耗尽时只提示改用文字，不自动购买资源包、升级CloudBase标准版或开启后付费。CloudBase Agent内置语音能力要求标准版及以上，本POC不采用该路径。

**2026-07-19外部状态与部署证据：**账号持有人已开通中国大陆语音识别，控制台明确显示国内后付费关闭、跨境流量未开通且跨境后付费关闭。开发者工具CLI在匹配环境列出`asr/conversations/entries`后，成功更新部署`asr`；随后将远端`asr`执行超时从平台默认3秒调整为20秒，并由CLI核验为`Active / timeout=20 / Nodejs16.13`。开发者工具重新编译为0错误，主体受限插件和无效`scope.record`声明均已移除。

**待办/待验证：**在物理真机验证录音权限、转写、AI回复和临时录音删除。为避免未经确认采集电脑周围声音，本轮没有自动打开模拟器麦克风。完成前只称云端部署与编译完成，不称语音端到端验收完成。

**2026-07-19 内容安全调用补丁与复测：**开发者工具真实发送曾确认请求能到达`conversations`，但云端统一返回“内容安全检查暂时不可用”。根因是函数使用`cloud.openapi.security.msgSecCheck`却缺少同目录`config.json`的`permissions.openapi`声明；已补充`security.msgSecCheck`权限、加入契约测试，并通过远端安装依赖方式重新部署`conversations`（16个文件）。部署后在开发者工具重新发送消息，页面已收到`hy3`助手回复，“内容安全检查暂时不可用”不再出现，证明输入审核、云函数、模型与页面回复链路恢复。内容审核继续失败关闭，没有使用跳过审核的临时绕过。

**2026-07-19 语音模拟器复测：**开发者工具可触发原生录音，控制台保持0错误，旧的`atob`无效音频异常未再出现。开发者工具明确警告其录音文件格式与移动端不同，本次模拟器录音无法完成转换；前端已为录音上传（15秒）和ASR调用（30秒）增加超时降级，复测确认“转换中”能自动消失并恢复文字输入。真实语音内容、真机录音格式、ASR转写、转写后复用文字发送及临时文件删除仍须物理真机验收。

**2026-07-19 语音回调根因与完整复测：**用户真机反馈“未获取到录音”后，确认前端错误地使用`recorder.onStop = callback`和`recorder.onError = callback`覆盖API方法，导致停止事件未注册、临时文件路径永远无法返回。现已改为`recorder.onStop(callback)`/`onError(callback)`并在每轮后通过`offStop`/`offError`解除监听，防止重复回调。开发者工具重新编译为0错误；使用按住说话后，真实ASR转写出“你好”，页面自动复用`send()`并收到助手回复，证明“录音停止→临时文件→上传→ASR→内容安全→conversations→hy3→页面回复”模拟器链路完整通过。7.3仍保留物理真机复测，以确认设备权限、移动端AAC和录音删除。

**2026-07-19 麦克风释放加固：**完整复测后，macOS状态栏仍显示微信开发者工具占用麦克风；终止模拟器页面后占用提示未立即消失，说明这是开发者工具音频会话没有及时释放，不能据此判断ASR仍在处理或小程序仍处于录音状态。前端现会在录音完成的`finally`、页面隐藏和页面卸载时统一停止录音、解除`onStop/onError`监听并清空RecorderManager引用；关闭开发者工具窗口用于终止本次桌面占用。相关语音链路测试、TypeScript/项目检查和OpenSpec严格校验均通过。是否已释放仍以macOS橙色麦克风指示实际熄灭为准；若窗口关闭后仍亮，应从应用菜单完整退出微信开发者工具。物理真机仍需确认松手即停、转写与回复正常，且退出页面后系统录音指示立即消失。

**2026-07-23 一句话识别权限与错误分类修复：**腾讯云官方文档确认`SentenceRecognition`由云函数运行角色的临时SecretId/SecretKey/SessionToken鉴权，腾讯云账号AppID不需要写入API请求或自定义环境变量；运行角色只需授予操作级`asr:SentenceRecognition`，资源为`*`。本地新增最小策略模板`cloudbase/asr-runtime-role-policy.example.json`，不保存长期密钥。原实现把所有`FailedOperation.*`统一映射成“服务未开通”，导致额度耗尽、欠费隔离和识别失败都显示同一句误导文案；现已按官方错误码分别映射未开通、权限、临时凭证、额度、欠费和无效音频，并在云函数日志记录脱敏后的平台错误码与RequestId。语音专项测试4/4、TypeScript和OpenSpec严格校验通过；全量测试58/61，失败的两项发布硬化红灯和一项信息流Token测试均不在本次ASR范围，`npm run check`还被既有的conversation部署副本不一致阻塞。本次代码与运行角色策略尚未部署到真实环境，部署和真机复测前不得称为端到端打通。用户在聊天中暴露过长期凭证标识，未写入任何文件，必须在腾讯云访问管理中吊销并轮换，后续不得通过聊天传递SecretKey。

**2026-07-23 真实环境部署与最小角色验证（覆盖上一段部署边界）：**已核对当前CloudBase体验环境与MyAlly小程序账号归属一致，并修正本机小程序的旧环境指向。腾讯云CAM中已创建独立自定义策略和SCF运行角色，策略仅允许操作级`asr:SentenceRecognition`、资源`*`；远端`asr`已部署为`Nodejs20.19`并绑定该角色，CLI最终核验`Active / Available / timeout=20`。绑定前使用合成音频调用得到`AuthFailure.UnauthorizedOperation`，绑定后使用同一角色再次调用成功转写“Hello, my alley voice test.”，因此服务开通、临时凭证注入、最小权限和SentenceRecognition调用链已取得远端证据。两次验证使用的一次性函数均已删除，未开启后付费、自动续费或跨境流量。仍未完成物理真机的麦克风授权、移动端AAC、转写后复用发送、AI回复及临时文件删除验收；不得用合成音频冒烟代替真机7.3/7.6。
...
## 11. 产品方向记录（2026-07-21）

### 技能专区定位（已确认）

结论：精选 tab 的技能板块从"外部AI工具导航(复制链接)"改为"内置技能专区"——预设角色的能力包，点击直接在小程序内使用，用完即走，形态参考腾讯元宝专家列表(2列宫格)。
依据：用户明确指示"技能最好直接可以用，而不是跳到外面"；"相当于给分身加一些能力"。同时绕开微信运营规范5.10对外部跳转的限制。
影响：技能=systemPrompt+welcomeMessage 的预设包，经服务端系统提示词追加注入，不落库不进记忆；后续新技能只需扩充 data/skills.json。
下一步验证：真机点击进聊天、开场白、角色化回答质量。

### AI试衣间（候选，未排期）

结论：用户提出后续加"AI试衣间"——上传线下衣服和本人真实照片，在小程序内虚拟试穿。
