...
const { batchSemanticScore } = require('./embedder');
...
async function hybridRetrieve({ query, items, maxItems = 8, maxChars = 2400 }) {
  ...
  // 生成语义分数（批调用 Kimi K3 判断相关性）
  let semanticScores = [];
  try {
    semanticScores = await batchSemanticScore(query || '', items);
  } catch {
    semanticScores = items.map(() => 0);
  }
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const finalScore = 0.20 * kw + 0.40 * sem + 0.20 * fts + 0.10 * rec + 0.10 * tb;
    if (finalScore > 0) {
