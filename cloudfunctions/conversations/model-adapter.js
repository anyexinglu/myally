...
type（stable_fact/current_state/preference/goal/decision_rule/relationship_boundary/action_result）、value、keywords、sourceType、confidence、sensitivity。
...
class CloudBaseModelAdapter {
  constructor({ ai, provider = 'cloudbase', modelName = 'glm-5v-turbo', observerModelName = '' }) {
    this.model = ai.createModel(provider);
    this.modelName = modelName;
    this.observerModelName = observerModelName || modelName;
  }
...
不要输出Markdown代码围栏。`;
    ...
    const generated = await this.complete(messages, { temperature: 0.2 });
