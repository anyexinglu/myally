...
CloudBase ModelAdapter继续通过环境变量选择provider/model。`MYALLY_MODEL_NAME`是兼容回退；可选的fast、reasoner、multimodal、observer模型分别处理普通/事实问题、个人复杂建议、图片和记忆抽取。未配置专用模型时全部回退到基础模型，因此单模型环境不增加部署门槛。实时搜索通过可替换HTTP适配器和环境变量启用，不把密钥传给小程序或写入日志。
