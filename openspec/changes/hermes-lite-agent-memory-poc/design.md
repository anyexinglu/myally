...
- `messages(ownerId, requestId, role)`唯一索引，保证每轮最多一条用户消息和一条助手消息；
- `profile_items(ownerId, status, deletedAt, updatedAt)`；
- `profile_items(ownerId, key, status)`；
...
运行时必须校验工具参数的JSON Schema，包括必填字段、类型、长度和额外字段。第`maxSteps`个模型步骤若仍请求工具，拒绝该请求且不执行工具，因为已没有剩余模型步骤可以消费工具结果；这避免无结果调用和额外费用。
...
Memory Observer只接收本轮原始用户消息及其不透明sourceMessageId，不接收助手回复、召回上下文或工具输出。模型输出限定为JSON数组；每项包含稳定语义`key`、`type`、`value`、`keywords`、`sourceType`、`confidence`和`sensitivity`。

每项先写append-only Observation。仅`explicit_user_statement`或`explicit_user_correction`可升格为`confirmed` Profile Item；`model_inference`保持candidate且不能进入召回。Profile Item保留`key`、`sourceMessageId`、`observedAt`、`validFrom/validTo`、`extractorVersion`和删除状态。显式纠正会在同一owner和语义key内关闭旧confirmed版本，将其标为`superseded`并设置`validTo`，再写入新confirmed版本；历史仍可审计，但检索只返回当前版本。Observer失败不影响主回复，响应中明确`memoryStatus=failed`。

### 7. 检索和上下文预算

Retriever只接受confirmed、未删除和当前有效记录，用查询关键词、记忆关键词、类型和更新时间进行小数据量重排，最多返回8条/2400字符。零相关分数的条目必须排除；仅当用户明确要求结合个人情况、方案、计划或下一步时，才允许把偏好、目标和决策规则作为弱相关候选。注入块标记`do_not_store=true`，同时把实际使用的opaque记忆ID返回小程序。完整画像和candidate不得注入。
...
CloudBase ModelAdapter继续通过环境变量选择provider/model。基础默认使用当前成长计划可用的`cloudbase / hy3`完成最低配置冒烟，`MYALLY_MODEL_NAME`可覆盖该回退；可选的fast、reasoner、multimodal、observer模型分别处理普通/事实问题、个人复杂建议、图片和记忆抽取。未配置专用模型时全部回退到基础模型，因此单模型环境不增加部署门槛；专用模型必须先在当前环境启用，不能把未开通的收费模型静默设为默认。实时搜索通过可替换HTTP适配器和环境变量启用，不把密钥传给小程序或写入日志。
